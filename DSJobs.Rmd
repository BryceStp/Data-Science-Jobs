...
data$R <- ifelse(sapply(data$Job.Description, function(x) Skills[1:3][str_detect(x, Skills[1:3])])  %in% Skills[1:3],1,0)

data$C <- ifelse(sapply(data$Job.Description, function(x) Skills[4:6][str_detect(x, Skills[4:6])])  %in% Skills[4:6],1,0)

# Loop through all values to create dummy variables (Note that this doesn't cover every skill that might pop up)
for(i in 7:63) {                                  
  new <- ifelse(sapply(data$Job.Description, function(x) Skills[i][str_detect(x, Skills[i])])  %in% Skills[i],1,0)
